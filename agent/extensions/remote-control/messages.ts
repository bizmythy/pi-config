...
export function serializeMessage(id: string, msg: RawMessage): RenderMsg | null {
  if (msg.role === "user") {
    const text =
...
export function getBranchMessages(ctx: ExtensionContext): RenderMsg[] {
  ...
  for (const entry of branch) {
    ...
    const m = serializeMessage(entry.id, (entry as { id: string; type: string; message: RawMessage }).message);
    if (m) out.push(m);
  }
  return out;
}
