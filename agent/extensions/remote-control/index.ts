...
import { buildRemoteControlUrl, configureRemoteControlUI, readRemoteControlConfig } from "./config.js";
...
export default function remoteControl(pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (!publicBaseUrl) {
      if (ctx.hasUI) {
        ctx.ui.notify("--remote-control: no publicBaseUrl configured. Run /remote-control config first.", "warning");
      }
      return;
    }
  ...
  pi.on("message_update", async (event) => {
    const m = serializeMessage("pending", (event as { message: RawMessage }).message);
  ...
  pi.registerCommand("remote-control", {
    ...
    handler: async (_args, ctx) => {
      ...
      const menuItems = [isRunning ? "Turn off" : "Turn on", configLabel, ...(isRunning ? ["Status"] : [])];
