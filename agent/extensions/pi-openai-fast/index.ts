 ...
 *
 * `/fast` and `--fast` toggle `service_tier=priority` for supported OpenAI models.
 ...
 *
 * Built-in support follows OpenAI Priority processing model families for
 * OpenAI/OpenAI Codex providers. `supportedModels` can add explicit
 * provider/model-id pairs without replacing the built-in model detection.
 */
...
const OPENAI_PRIORITY_PROVIDERS = new Set(["openai", "openai-codex"]);
const OPENAI_PRIORITY_MODEL_PATTERN = /^gpt-5(?:\.\d+)?(?:-(?:pro|mini|nano))?$/;
const DEFAULT_SUPPORTED_MODEL_KEYS = [] as const;
...
function isDefaultFastSupportedModel(model: ExtensionContext["model"]): boolean {
  if (!model) {
    return false;
  }
  return OPENAI_PRIORITY_PROVIDERS.has(model.provider) && OPENAI_PRIORITY_MODEL_PATTERN.test(model.id);
}

function isConfiguredFastSupportedModel(
...
function isFastSupportedModel(model: ExtensionContext["model"], supportedModels: FastSupportedModel[]): boolean {
  return isDefaultFastSupportedModel(model) || isConfiguredFastSupportedModel(model, supportedModels);
}

function describeSupportedModels(supportedModels: FastSupportedModel[]): string {
  const builtIn = "OpenAI/OpenAI Codex GPT-5 priority models";
  if (supportedModels.length === 0) {
    return builtIn;
  }
  return `${builtIn}; configured extras: ${supportedModels.map((supported) => `${supported.provider}/${supported.id}`).join(", ")}`;
}
...
export default function piOpenAIFast(pi: ExtensionAPI): void {
  ...
  pi.registerCommand(FAST_COMMAND, {
    description: "Toggle fast mode (priority service tier for supported OpenAI models)",
...
export const _test = {
  ...
  FAST_COMMAND_ARGS,
  FAST_SERVICE_TIER,
  OPENAI_PRIORITY_PROVIDERS,
  OPENAI_PRIORITY_MODEL_PATTERN,
  ...
  readConfigFile,
  resolveFastConfig,
  isDefaultFastSupportedModel,
  isConfiguredFastSupportedModel,
