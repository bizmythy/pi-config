...
function isCommandContext(ctx: ExtensionContext): ctx is ExtensionCommandContext {
  return (
    typeof (ctx as ExtensionCommandContext).waitForIdle === "function" &&
...
async function clearSession(ctx: ExtensionCommandContext) {
  ...
  const result = await ctx.newSession({
    withSession: async (ctx) => {
      ctx.ui.notify("Started a fresh session", "success" as "info");
    },
  });
  ...
  pi.registerShortcut("ctrl+n", {
    ...
    handler: async (ctx) => {
      if (!isCommandContext(ctx)) {
        ctx.ui.notify(
          "Use /clear to start a fresh session; shortcuts cannot switch sessions in this pi version.",
          "warning",
        );
        return;
      }
