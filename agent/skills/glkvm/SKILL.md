...
compatibility: Requires uv, network access to kvm.home.drewcouncil.com, and SecretSpec.
---
...
- Credentials: the `glkvm` profile in the repo's `secretspec.toml`
- Deterministic CLI: `scripts/glkvm.py`

Do not ask for the address, username, or password, and do not use the curl examples below for real interactions. The CLI resolves both fields through the SecretSpec Python SDK, never prints or persists the password, and caches only GLKVM session cookies.

## Initialization

First try a read-only command, which reuses the cached session and does **not** resolve credentials:
